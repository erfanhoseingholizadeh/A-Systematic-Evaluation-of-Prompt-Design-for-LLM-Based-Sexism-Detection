...
\textsuperscript{$\dagger$}llama3.1's V14 omission and the V8/V13/V15
caveat are explained in Sec.~\ref{sec:results:llama-refusal}.}
\label{tab:main-grid}
...
variants. Refusal share is the fraction of a variant's failed rows
matching an explicit decline pattern (e.g.\ ``I cannot,'' ``not able
to''); the remainder are token-budget truncations, not declines (see
text).}
\label{tab:llama-cot}
\begin{tabular}{lcccc}
\toprule
...
Five of the six models' best training-free prompt variant, after
